...
<h3>Ustawienia sensorów temperatury:</h3>
	...
	<tr>
	    <td>
		<input class="medium" {if $sensor.interface_disabled eq 1}style="color:grey" readonly="readonly"{/if} name="sensors[{$sensor.interface_id}][sensor_name]" value="{$sensor.interface_name}"
	    ...
	    <td>
		<select class="short" name="sensors[{$sensor.interface_id}][sensor_unit]" id="sensors[{$sensor.interface_id}][sensor_unit]" {if $sensor.interface_disabled eq 1}disabled style="color:grey"{/if}
			onmouseover="return overlib('Wybierz jednostkę.');"
			onmouseout="return nd();">
		    <option value="none">Brak</option>
		    {foreach from=$units item=unit}
			<option {if $sensor.interface_unit.unit_id eq $unit.unit_id}selected="selected"{/if} value="{$unit.unit_id}">{$unit.unit_name}</option>
		    {/foreach}
		</select>
	    </td>
	...
	<tr>
	    <td>
		<input class="medium" name="sensors[{$new_interface_id}][sensor_name]" value=""
	    ...
	    <td>
		<select class="short" name="sensors[{$new_interface_id}][sensor_unit]" id="sensors[{$new_interface_id}][sensor_unit]"
			onmouseover="return overlib('Wybierz jednostkę.');"
			onmouseout="return nd();">
		    <option value="none">Brak</option>
		    {foreach from=$units item=unit}
